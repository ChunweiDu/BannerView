package com.dcw.bannerviewpager.BannerView;
...
/**
 * @Author： duchunwei
 * @Date: 2017/6/14   15:52
 * @Email： duchunwei_it@163.com
 */

public class MRBannerView<T> extends RelativeLayout {

    private ViewPager mViewPager;
    private MRPagerAdapter mAdapter;
    ...
    private int mMRModePadding = 0;//在仿魅族模式下，由于前后显示了上下一个页面的部分，因此需要计算这部分padding
    ...
    public MRBannerView(@NonNull Context context) {
        super(context);
        init();
    }

    public MRBannerView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        readAttrs(context, attrs);
        init();
    }

    public MRBannerView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    public MRBannerView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr, @StyleRes int defStyleRes) {
    ...
    private void readAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MRBannerView);
        mIsOpenMZEffect = typedArray.getBoolean(R.styleable.MRBannerView_open_mz_mode, true);
        mIsCanLoop = typedArray.getBoolean(R.styleable.MRBannerView_canLoop, true);
        mIndicatorAlign = typedArray.getInt(R.styleable.MRBannerView_indicatorAlign, 1);
        mIndicatorPaddingLeft = typedArray.getDimensionPixelSize(R.styleable.MRBannerView_indicatorPaddingLeft, 0);
        mIndicatorPaddingRight = typedArray.getDimensionPixelSize(R.styleable.MRBannerView_indicatorPaddingRight, 0);
    }
    ...
    private void init() {
        View view = null;
        if (mIsOpenMZEffect) {
            view = LayoutInflater.from(getContext()).inflate(R.layout.mr_banner_effect_layout, this, true);
        } else {
            view = LayoutInflater.from(getContext()).inflate(R.layout.mr_banner_normal_layout, this, true);
        }
    ...
    private void initIndicator() {
        ...
        for (int i = 0; i < mDatas.size(); i++) {
            ...
            if (mIndicatorAlign == IndicatorAlign.LEFT.ordinal()) {
                if (i == 0) {
                    int paddingLeft = mIsOpenMZEffect ? mIndicatorPaddingLeft + mMRModePadding : mIndicatorPaddingLeft;
                ...
                if (i == mDatas.size() - 1) {
                    int paddingRight = mIsOpenMZEffect ? mMRModePadding + mIndicatorPaddingRight : mIndicatorPaddingRight;
    ...
    /**
     * 开始轮播
     * <p>应该确保在调用用了{@link MRBannerView {@link #setPages(List, MRHolderCreator)}} 之后调用这个方法开始轮播</p>
     */
    public void start() {
     ...
     * @param mzHolderCreator ViewHolder生成器 {@link MRHolderCreator} And {@link MRViewHolder}
     */
    public void setPages(List<T> datas, MRHolderCreator mzHolderCreator) {
        ...
        mAdapter = new MRPagerAdapter(datas, mzHolderCreator, mIsCanLoop);
    ...
    public static class MRPagerAdapter<T> extends PagerAdapter {
        private List<T> mDatas;
        private MRHolderCreator mMZHolderCreator;
        private ViewPager mViewPager;
        private boolean canLoop;
        private BannerPageClickListener mPageClickListener;
        private final int mLooperCountFactor = 500;

        public MRPagerAdapter(List<T> datas, MRHolderCreator MZHolderCreator, boolean canLoop) {
            if (mDatas == null) {
